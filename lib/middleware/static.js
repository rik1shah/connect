...
var send = exports.send = function(req, res, next, options){
  ...
  fs.stat(path, function(err, stat){
    ...
    if (fn) {
      ...
      function reqClose(err){console.log("req closed, res written:" + res.socket.bytesWritten + " bytes read:" + res.socket.bytesRead); callback(err)}
      function socketError(err){console.log("socket Errored, res written:" + res.socket.bytesWritten + " bytes read:" + res.socket.bytesRead); callback(err)}
      function streamError(err){console.log("stream Errored, res written:" + res.socket.bytesWritten + " bytes read:" + res.socket.bytesRead); callback(err)}
      function streamEnd(err){console.log("stream Ended, res written:" + res.socket.bytesWritten + " bytes read:" + res.socket.bytesRead); callback(err)}
      req.on('close', reqClose);
      req.socket.on('error', socketError);
